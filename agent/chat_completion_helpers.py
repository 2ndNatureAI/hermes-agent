...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None, reset_at=None) -> bool:
    ...
    while True:
        ...
        try:
            ...
            # Re-publish the resolved route: the kanban run-fields stamp must record
            # the fallback route this run actually used, not the primary it started on.
            try:
                from agent.conversation_loop import _publish_resolved_state
                _publish_resolved_state(agent)
            except Exception:
                logger.debug("fallback resolved-state republish failed", exc_info=True)
